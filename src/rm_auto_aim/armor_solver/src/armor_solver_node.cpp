...
: Node("armor_solver", options), solver_(nullptr) {
  ...
  r_x_ = declare_parameter("ekf.r_x", 0.03);
  r_y_ = declare_parameter("ekf.r_y", 0.03);
  r_z_ = declare_parameter("ekf.r_z", 0.03);
...
void ArmorSolverNode::timerCallback() {
  ...
  if (armor_target_.tracking) {
    try {
      ...
      //敌方小陀螺速度大于0.95rad/s时瞄准中心
      if (armor_target_.v_yaw > 0.95)
      {
      control_msg.yaw = std::atan2(armor_target_.position.y,armor_target_.position.x) * 180 / M_PI;
      }
      // control_msg.yaw = std::atan2(armor_target_.position.y,armor_target_.position.x) * 180 / M_PI;
    } catch (...) {
