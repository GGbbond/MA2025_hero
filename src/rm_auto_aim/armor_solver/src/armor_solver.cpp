...
Solver::Solver(std::weak_ptr<rclcpp::Node> n) : node_(n) {
  ...
  shooting_range_w_ = node->declare_parameter("solver.shooting_range_width", 0.050);
  shooting_range_h_ = node->declare_parameter("solver.shooting_range_height", 0.050);
...
rm_interfaces::msg::GimbalCmd Solver::solve(const rm_interfaces::msg::Target &target,
  ...
  // FYT_ERROR("armor_solver","debug");
  if (target.v_yaw > 0.95){     //判断目标小陀螺转速是否大于0.95rad/s
    double odom_x = 0.0;
    double odom_y = 0.0;
    double center_x = target_position.x();
    double center_y = target_position.y();
    double armor_x = chosen_armor_position.x();
    double armor_y = chosen_armor_position.y();
    Eigen::Vector2d Center2Odom(odom_x - center_x, odom_y - center_y);
    Eigen::Vector2d Center2Armor(armor_x - center_x, armor_y - center_y);
    double dot = Center2Odom.dot(Center2Armor);
    double Center2OdomNorm = Center2Odom.norm();
    double Center2ArmorNorm = Center2Armor.norm();
    double ArmorAndOdom2CenterAngle = std::acos(dot / (Center2OdomNorm * Center2ArmorNorm)) / M_PI * 180;
    FYT_WARN("armor_solver","{}",ArmorAndOdom2CenterAngle);
    gimbal_cmd.fire_advice = std::abs(ArmorAndOdom2CenterAngle) < 0.1 && isOnTarget(rpy_[2], rpy_[1], yaw, pitch, distance);  //当目标的yaw轴角度的绝对值小于0.3弧度时并且满足瞄准条件时开火
  }else
  {
  ...
  // double Center2ArmorNorm = Center2Armor.norm();
  // double ArmorAndOdom2CenterAngle = std::acos(dot / (Center2OdomNorm * Center2ArmorNorm)) / M_PI * 180;
  // gimbal_cmd.fire_advice = std::abs(ArmorAndOdom2CenterAngle) < 1 && isOnTarget(rpy_[2], rpy_[1], yaw, pitch, distance);  //当目标的yaw轴角度的绝对值小于0.3弧度时并且满足瞄准条件时开火
